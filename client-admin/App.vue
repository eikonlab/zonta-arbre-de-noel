  ...
  <div class="admin-container">
    ...
    <div class="controls">
      ...
      <div class="action-controls">
        <button @click="refreshMessages" class="btn btn-primary">
          🔄 Actualiser
        </button>
        <button @click="bulkAction('show')" class="btn btn-success">
          👁️ Tout afficher
        </button>
        <button @click="bulkAction('hide')" class="btn btn-danger">
          🙈 Tout masquer
        </button>
      </div>
    </div>
          ...
          <div class="message-actions">
            <label class="visibility-toggle">
              ...
              <span class="toggle-label">
                {{ message.hidden ? "🙈 Masqué" : "👁️ Visible" }}
              </span>
            </label>
...
export default {
  ...
  methods: {
    ...
    async bulkAction(action) {
      ...
      try {
        const updates = this.messages.map((message) => ({
          id: message.id,
          hidden: action === "hide",
        }));

        await axios.patch("http://localhost:3001/messages/bulk", { updates });
